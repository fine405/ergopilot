...
The `/lab` route drives the existing Rust simulator fault paths and renders
their recovery evidence. The `/evals` route reads schema-validated published
and local planner reports through the Hono control plane. Neither route gives
the browser direct access to SQLite or device RPC method names.
