...
Current milestone: the Hono control plane atomically persists the latest 100
...
The local store is contract-validated at startup and survives a control-plane
restart. Prompts, user identifiers and credentials are excluded. The Web
operator console renders the same privacy-safe evidence and refreshes it after
planning. A deterministic scorer now provides a six-case smoke suite and an
explicit 30-case dataset covering English/Chinese intent, unit and policy
